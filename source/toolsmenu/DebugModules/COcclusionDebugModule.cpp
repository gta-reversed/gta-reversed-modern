...
void COcclusionDebugModule::RenderWindow() {
    const notsa::ui::ScopedWindow window{ "Occlusion", {}, m_IsOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize };
    if (!m_IsOpen) {
        return;
    }
...
void COcclusionDebugModule::RenderMenuEntry() {
    notsa::ui::DoNestedMenuIL({ "Visualization", "Occls" }, [&] {
