...
/*!
* This function is used for the camera anti-clipping
* The input to this function is basically generated by `SphereCastVsEntity`
* The output format is exactly the same as the input, that is,
* it consists of regions.
* Each region's first entry has the `ent` (entity) set, and
* all following entries that don't have it set belong to it.
* @addr 0x4181B0
*/
bool CCollision::SphereCastVsCaches(
    ...
    if (numToTest && !toTest[0].ent) {
        DEV_LOG("Problems have occured!"); // OG bug, we don't have to worry about it though
    }
    //assert(!numToTest || toTest[0].ent); 

    // Process entities now
    // This shitty array (toTest) has regions.
    // Each region starts with an entry that has `ent` set.
    // All following entry (up until the next such entry) belongs to it.
    for (auto i = 0; i < numToTest; ) {
        const auto entity = toTest[i].ent;
        if (!entity) {
        next:
            i++;
            continue;
        ...
        for (;; i++) {
            ...
            if (i + 1 >= numToTest) { // No more entries
                goto finished;
            }

            if (toTest[i + 1].ent) { // Found new region
                goto next;
            }
        }
