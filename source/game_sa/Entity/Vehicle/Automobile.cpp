...
bool CAutomobile::ProcessAI(uint32& extraHandlingFlags)
{
    ...
    case STATUS_PLANE:
        if (m_vecMoveSpeed.SquaredMagnitude() < 0.01f
            || m_pDriver
            && m_pDriver->IsPlayer()
            && (m_pDriver->m_nPedState == PEDSTATE_ARRESTED || m_pDriver->GetTaskManager().Has<TASK_COMPLEX_CAR_SLOW_BE_DRAGGED_OUT, TASK_COMPLEX_CAR_QUICK_BE_DRAGGED_OUT, TASK_SIMPLE_CAR_WAIT_TO_SLOW_DOWN>()))
        {
            m_fBreakPedal = 1.0f;
            m_fGasPedal = 0.0f;
            vehicleFlags.bIsHandbrakeOn = true;
