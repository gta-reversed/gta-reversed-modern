...
void CTaskSimpleGoToPointFine::SetBlendedMoveAnim(CPed* ped)
{
...
#else
    auto pIdleAnimAssoc = RpAnimBlendClumpGetAssociation(ped->m_pRwClump, 3);
    auto pWalkAnimAssoc = RpAnimBlendClumpGetAssociation(ped->m_pRwClump, static_cast<unsigned int>(0));
    auto pRunAnimAssoc = RpAnimBlendClumpGetAssociation(ped->m_pRwClump, 1);
    auto pSprintAnimAssoc = RpAnimBlendClumpGetAssociation(ped->m_pRwClump, 2);
    auto pIdleTiredAnimAssoc = RpAnimBlendClumpGetAssociation(ped->m_pRwClump, 10);
    ...
    if (m_fBlend == 0.0f) {
        if (!pIdleAnimAssoc)
            CAnimManager::BlendAnimation(ped->m_pRwClump, ped->m_nAnimGroup, 3u, 4.0f);
        if (pWalkAnimAssoc)
    ...
    if (m_fBlend > 1.0f) {
        if (m_fBlend > 2.0f) {
            ...
            if (pWalkAnimAssoc)
                delete pWalkAnimAssoc;
            if (!pRunAnimAssoc) {
                pRunAnimAssoc = CAnimManager::AddAnimation(ped->m_pRwClump, ped->m_nAnimGroup, 1);
            ...
            if (!pSprintAnimAssoc)
            {
                pSprintAnimAssoc = CAnimManager::AddAnimation(ped->m_pRwClump, ped->m_nAnimGroup, 2);
            ...
            if (!pWalkAnimAssoc) {
                pWalkAnimAssoc = CAnimManager::AddAnimation(ped->m_pRwClump, ped->m_nAnimGroup, 0);
