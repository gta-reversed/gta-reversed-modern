...
public:
    ...
    CTaskSimpleIKChain(const char* name, ePedBones effectorBoneTag, RwV3d effectorVec, ePedBones pivotBoneTag, CEntity* entity, ePedBones offsetBoneTag, RwV3d offsetPos,
    ...
    CTaskSimpleIKChain* Clone() override {
        return new CTaskSimpleIKChain{*this};
    }
    eTaskType GetTaskType() override {
        return TASK_SIMPLE_IK_CHAIN;
    }
...
private:
    CTaskSimpleIKChain* Constructor(char* name, ePedBones effectorBoneTag, RwV3d effectorVec, ePedBones pivotBoneTag, CEntity* a6, ePedBones offsetBoneTag, RwV3d offsetPos,
                                    float speed, int32 time, int32 blendTime);
    CTaskSimpleIKChain* Destructor();

    CTask* Clone_Reversed() {
        return CTaskSimpleIKChain::Clone();
    }
    eTaskType GetTaskType_Reversed() {
        return CTaskSimpleIKChain::GetTaskType();
    }
    bool MakeAbortable_Reversed(CPed* ped, eAbortPriority priority, CEvent const* event) {
        return CTaskSimpleIKChain::MakeAbortable(ped, priority, event);
    }
    bool ProcessPed_Reversed(CPed* ped) {
        return CTaskSimpleIKChain::ProcessPed(ped);
    }
    bool CreateIKChain_Reversed(CPed* ped) {
        return CTaskSimpleIKChain::CreateIKChain(ped);
    }
};
