...
void CTaskSimpleSwim::InjectHooks() {
    Install("CTaskSimpleSwim", "CTaskSimpleSwim", 0x688930, &CTaskSimpleSwim::Constructor);
    Install("CTaskSimpleSwim", "Clone", 0x68B050, &CTaskSimpleSwim::Clone_Reversed);
    Install("CTaskSimpleSwim", "GetTaskType", 0x6889F0, &CTaskSimpleSwim::GetId_Reversed);
    Install("CTaskSimpleSwim", "ProcessPed", 0x68B1C0, &CTaskSimpleSwim::ProcessPed_Reversed);
    Install("CTaskSimpleSwim", "MakeAbortable", 0x68B100, &CTaskSimpleSwim::MakeAbortable_Reversed);
    Install("CTaskSimpleSwim", "ApplyRollAndPitch", 0x68A8E0, &CTaskSimpleSwim::ApplyRollAndPitch);
    Install("CTaskSimpleSwim", "ProcessSwimAnims", 0x6899F0, &CTaskSimpleSwim::ProcessSwimAnims);
    Install("CTaskSimpleSwim", "ProcessSwimmingResistance", 0x68A1D0, &CTaskSimpleSwim::ProcessSwimmingResistance);
    Install("CTaskSimpleSwim", "ProcessEffects", 0x68AA70, &CTaskSimpleSwim::ProcessEffects);
    Install("CTaskSimpleSwim", "ProcessControlAI", 0x689640, &CTaskSimpleSwim::ProcessControlAI);
    Install("CTaskSimpleSwim", "ProcessControlInput", 0x688A90, &CTaskSimpleSwim::ProcessControlInput);
    Install("CTaskSimpleSwim", "CreateFxSystem", 0x68A9F0, &CTaskSimpleSwim::CreateFxSystem);
    Install("CTaskSimpleSwim", "DestroyFxSystem", 0x68AA50, &CTaskSimpleSwim::DestroyFxSystem);
}
...
bool CTaskSimpleSwim::ProcessPed_Reversed(CPed* pPed)
{
    ...
    if (pPed->IsPlayer())
    {
        if (m_nTimeStep && m_nSwimState != SWIM_UNDERWATER_SPRINTING)
        {
            ...
            uint32 swimmingTimeStep = static_cast<uint32>((CTimer::GetTimeStepInSeconds()) * 1000.0f);
...
void CTaskSimpleSwim::ProcessSwimmingResistance(CPed* pPed)
{
    ...
    case SWIM_BACK_TO_SURFACE:
    {
        ...
        if (pAnimAssociation) {
            if (pAnimAssociation->m_pHierarchy->m_fTotalTime > pAnimAssociation->m_fCurrentTime
                ...
                float fMoveForceZ = CTimer::GetTimeStep() * pPed->m_fMass * 0.3f * 0.008f;
    ...
    CVector vecCheckWaterLevelPos = CTimer::GetTimeStep() * pPed->m_vecMoveSpeed + pPed->GetPosition();
...
void CTaskSimpleSwim::ProcessControlAI(CPed* pPed)
{
    ...
    {
        if (!((pPed->m_nRandomSeedUpperByte + CTimer::GetFrameCounter() - 4) & 0x7F)) {
    ...
    case SWIM_SPRINTING:
    {
        ...
        if (CGameLogic::IsPlayerUse2PlayerControls(pPed))
        {
            ...
            if (fWalkMagnitude > 0)
            {
                ...
                pPed->m_fAimingRotation = fCurrenRotation * (CTimer::GetTimeStep() * 0.079999998f) + pPed->m_fCurrentRotation;
        ...
        if (!CCamera::m_bUseMouse3rdPerson)
        {
            if (fWalkMagnitude <= 0.0f)
            {
                ...
                {
                    pPlayerData->m_fMoveBlendRatio -= CTimer::GetTimeStep() * 0.05f;
                    if (pPlayerData->m_fMoveBlendRatio < 0.0f)
                    {
                        pPlayerData->m_fMoveBlendRatio = 0.0f;
                    }
            ...
            else
            {
                ...
                {
                    pPed->m_fAimingRotation += CTimer::GetTimeStep() * -0.03f * pedWalkX;
                    if (pPed->m_fAimingRotation <= DegreesToRadians(180.0f))
                    {
                        if (pPed->m_fAimingRotation < -DegreesToRadians(180.0f))
                        {
    ...
    case SWIM_UNDERWATER_SPRINTING:
    {
        ...
        else
        {
            if (fWalkMagnitude > 0.0f)
            {
                ...
                pPed->m_fAimingRotation += CTimer::GetTimeStep() * -0.029999999f * vecPedWalk.x;
