...
void CCredits::RenderCredits() {
    ...
    float m_CurrentOffset = float(CTimer::GetTimeInMilliseconds() - CreditsStartTime) * 0.0435f;
    
    CFont::SetBackground(false, false);
    CFont::SetCentreSize(SCREEN_WIDTH * 0.92f); // SCREEN_WIDTH / 23 or 24
    ...
    const auto PrintCreditText_ = [&](const char* textKey, float scale, bool highlighted) {
        ...
        PrintCreditText(scale, scale, text, m_Position, m_CurrentOffset, highlighted);
    };

    PrintCreditText_("CRED000", SCALE_BIG, true);
    PrintCreditSpace(SPACE_BIG, m_Position);
    ...
    PrintCreditSpace(SPACE_SMALL, m_Position);
    ...
    if (m_Position + SCREEN_HEIGHT - m_CurrentOffset < -10.0f) {
        bCreditsGoing = false;
    }
}
