...
void CCarEnterExit::InjectHooks() {
    ...
    RH_ScopedInstall(AddInCarAnim, 0x64F720, { .reversed = false });
    RH_ScopedInstall(CarHasDoorToClose, 0x64EE10, { .reversed = false });
    // RH_ScopedInstall(CarHasDoorToOpen, 0x0, { .reversed = false });
    // RH_ScopedInstall(CarHasOpenableDoor, 0x0, { .reversed = false });
    // RH_ScopedInstall(CarHasPartiallyOpenDoor, 0x0, { .reversed = false });
    // RH_ScopedInstall(ComputeDoorFlag, 0x0, { .reversed = false });
    // RH_ScopedInstall(ComputeOppositeDoorFlag, 0x0, { .reversed = false });
    RH_ScopedInstall(ComputePassengerIndexFromCarDoor, 0x64F1E0, { .reversed = false });
    RH_ScopedInstall(ComputeSlowJackedPed, 0x64F070, { .reversed = false });
    RH_ScopedInstall(ComputeTargetDoorToEnterAsPassenger, 0x64F190, { .reversed = false });
    RH_ScopedInstall(ComputeTargetDoorToExit, 0x64F110, { .reversed = false });
    RH_ScopedInstall(GetNearestCarDoor, 0x6528F0, { .reversed = false });
    ...
    // RH_ScopedInstall(IsCarDoorInUse, 0x0, { .reversed = false });
    // RH_ScopedInstall(IsCarDoorReady, 0x0, { .reversed = false });
    // RH_ScopedInstall(IsCarQuickJackPossible, 0x0, { .reversed = false });
    // RH_ScopedInstall(IsCarSlowJackRequired, 0x0, { .reversed = false });
    RH_ScopedInstall(IsClearToDriveAway, 0x6509B0, { .reversed = false });
    RH_ScopedInstall(IsPathToDoorBlockedByVehicleCollisionModel, 0x651210, { .reversed = false });
    RH_ScopedInstall(IsPedHealthy, 0x64EEE0, { .reversed = false });
    ...
    RH_ScopedInstall(IsVehicleHealthy, 0x64EEC0, { .reversed = false });
    RH_ScopedInstall(IsVehicleStealable, 0x6510D0, { .reversed = false });
    // RH_ScopedInstall(MakeUndraggedDriverPedLeaveCar, 0x0, { .reversed = false });
    // RH_ScopedInstall(MakeUndraggedPassengerPedsLeaveCar, 0x0, { .reversed = false });
    // RH_ScopedInstall(QuitEnteringCar, 0x0, { .reversed = false });
    RH_ScopedInstall(RemoveCarSitAnim, 0x64F680, { .reversed = false });
    RH_ScopedInstall(RemoveGetInAnims, 0x64F6E0, { .reversed = false });
    RH_ScopedInstall(SetAnimOffsetForEnterOrExitVehicle, 0x64F860, { .reversed = false });
    RH_ScopedInstall(SetPedInCarDirect, 0x650280, { .reversed = false });
}
