...
void CConversationForPed::Update() {
    ...
    if (CTimer::GetTimeInMS() > m_LastTimeWeWereCloseEnough + 10'000 ||
        ...
        if (!m_bSuppressSubtitles) {
            ...
            CMessages::AddMessageJump(TheText.Get(GetCurrentNode()->m_Name), 4000, 1, 1);
        }
        ...
        case eConversationForPedStatus::UNK_3: {
            ...
            if (pad->ConversationNoJustDown() && GetCurrentNode()->m_NodeNo >= 0) {
                if (!m_bSuppressSubtitles) {
                    CMessages::ClearSmallMessagesOnly();
                    CMessages::AddMessageJump(TheText.Get(std::format("{}N", GetCurrentNode()->m_Name).c_str()), 4000, 3, 1);
                }
                ...
                if (!m_bSuppressSubtitles) {
                    CMessages::ClearSmallMessagesOnly();
                    CMessages::AddMessageJump(TheText.Get(std::format("{}Y", GetCurrentNode()->m_Name).c_str()), 4000, 3, 1);
                }
            ...
            if (!m_bSuppressSubtitles) {
                CMessages::AddMessageQ(TheText.Get(GetCurrentNode()->m_Name), 4000, 1, 1);
            }
