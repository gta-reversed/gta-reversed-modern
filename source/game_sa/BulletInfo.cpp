...
// AddBullet
void CBulletInfo::AddBullet(CEntity* creator, eWeaponType weaponType, CVector pos, CVector velocity) {
    if (auto it = GetFree()) {
        it->m_pCreator = creator;
        it->m_nWeaponType = weaponType;
        it->m_nDamage = CWeaponInfo::GetWeaponInfo(weaponType, eWeaponSkill::WEAPSKILL_STD)->m_nDamage;
        it->m_vecPosition = pos;
        it->m_vecVelocity = velocity;
        it->m_nDestroyTime = (float)(CTimer::GetTimeInMS() + 1000);
        it->m_bExists = true;
    }
}
            ...
            case eEntityType::ENTITY_TYPE_PED: {
                ...
                if (CLocalisation::Blood()) {
                    ...
                    if (hitPed->m_nPedState == ePedState::PEDSTATE_DEAD) {
                        const auto animID = RpAnimBlendClumpGetFirstAssociation(hitPed->m_pRwClump, 0x800u) ? ANIM_ID_FLOOR_HIT_F : ANIM_ID_FLOOR_HIT;
                        if (auto anim = CAnimManager::BlendAnimation(hitPed->m_pRwClump, 0, animID, 8.0f)) {
                            anim->Start(0.0f);
                            anim->SetFlag(eAnimationFlags::ANIM_FLAG_UNLOCK_LAST_FRAME, false);
                            std::cout << "Blood anim\n";
                        }
                    }
                    newPosition = colPoint.m_vecPoint;
