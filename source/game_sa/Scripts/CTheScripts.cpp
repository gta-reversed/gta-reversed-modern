...
signed int CTheScripts::GetActualScriptThingIndex(int index, unsigned char type) {
    return plugin::CallAndReturn<signed int, 0x4839A0, int, unsigned char>(index, type);
}

// 0x483720
unsigned int CTheScripts::GetNewUniqueScriptThingIndex(unsigned int index, char type) {
    return plugin::CallAndReturn<unsigned int, 0x483720, unsigned int, char>(index, type);
}

// 0x464D20
int CTheScripts::GetScriptIndexFromPointer(CRunningScript* thread) {
    return plugin::CallAndReturn<int, 0x464D20, CRunningScript*>(thread);
}
