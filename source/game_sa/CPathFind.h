...
public:
    ...
    inline bool IsValid() { return *reinterpret_cast<unsigned short*>(this) != 0xFFFF; }
};
...
public:
    ...
    void DoPathSearch(unsigned char pathType, CVector origin, CNodeAddress originAddr, CVector target, CNodeAddress* pResultNodes, short* pNodesCount, int maxNodesToFind,
    ...
    CNodeAddress* FindNodeClosestToCoors(CNodeAddress* pathLink, float X, float Y, float Z, int _nodeType, float maxDistance, unsigned short unk2, int unk3, unsigned short unk4,
                                         unsigned short bBoatsOnly, int unk6);
