    ...
    void ProcessArgument(const char* arg) {
        ...
        if (str == "--unhook-all") {
            if (!unhookExcept.empty()) {
                NOTSA_LOG_DEBUG("warn: --unhook-except has been called previously, unhook-all will be effective.");
                unhookExcept.clear(); // we aren't gonna use it.
            }
            if (!unhookSome.empty()) {
                NOTSA_LOG_DEBUG("warn: --unhook-some has been called previously, unhook-all will be effective.");
        ...
        if (str.starts_with("--unhook-except=")) {
            if (unhookAll) {
                NOTSA_LOG_DEBUG("warn: --unhook-all has been called previously, unhook-all will be effective.");
            } else {
                if (!unhookSome.empty()) {
                    NOTSA_LOG_DEBUG("warn: --unhook-some has been called previously, unhook-except will be effective.");
                    unhookSome.clear(); // we aren't gonna use it.
                }
        ...
        if (str.starts_with("--unhook=")) {
            if (unhookAll) {
                ...
                NOTSA_LOG_DEBUG("warn: --unhook-except has been called previously, unhook-except will be effective.");
            } else {
                for (auto hook : SplitStringView(str.substr(str.find('=') + 1), ",")) {
        ...
        NOTSA_LOG_DEBUG("warn: unknown argument '{}'", str);
    }
